...
class somaFunctions extends somaticFramework {
    ...
    function fetch_facebook_pic($pid, $size = "square") {
        ...
        // $userurl = "//graph.facebook.com/$fid";
        // $user = json_decode(file_get_contents($userurl));
        // $feedurl = "//graph.facebook.com/$fid/feed";
    ...
    function get_current_url() {
        $urlis = '//' . $_SERVER['HTTP_HOST'] . htmlentities($_SERVER['PHP_SELF']) . $_SERVER['REQUEST_URI'];
        return $urlis;
    }
    ...
    function wp_version_check() {
        ...
        $new_admin_version = '4.4';
        $updateURL = "";
        if (version_compare($wp_version, $new_admin_version, '<')) {
            add_action( 'admin_notices', function() {
                echo '<div id=\"message\" class=\"error\" style=\"font-weight: bold\"><p>WORDPRESS 4.4 MINIMUM REQUIRED - please update WP or de-activate this plugin!</p></div>';
                }
            );
        }
    }
...
SQL;
    ...
    */
    ...
    // grabs metadata from public vimeo API, returns array --NOT USED??
    function fetch_vimeo_meta( $link = null ) {
        ...
        $oembed = json_decode(file_get_contents("https://vimeo.com/api/oembed.json?url=".urlencode($link)), true);
        $meta = array_shift(unserialize(file_get_contents("https://vimeo.com/api/v2/video/{$oembed['video_id']}.php")));
        $meta = array_merge($oembed, $meta);
        return $meta;
    }

    // grabs metadata from public youtube API, returns array --NOT USED??
    function fetch_youtube_meta( $url = null ) {
        ...
        $meta = json_decode(file_get_contents("https://gdata.youtube.com/feeds/api/videos/$id?v=2&alt=json"), true);     // json_decode true returns data as array instead of object
    ...
    function fetch_external_media($url = null, $width = null, $height = null) {
        ...
        // enforce https if user provided http
        ...
        switch ($site) {
            case "youtube";
                if (preg_match('%(?:youtube\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $url, $match)) {
                    ...
                    $media['url']       = "https://www.youtube.com/watch?v=$id";                                           // rebuild external url
                    $media['id']        = $id;
                    $media['site']      = $site;
                    ...
                    $media['embed']     = "<iframe width=\"$width\" height=\"$height\" src=\"//www.youtube.com/embed/$id?rel=0&hd=1\" frameborder=\"0\" allowfullscreen></iframe>";
                    $media['iframe']    = "//www.youtube.com/embed/$id?rel=0&hd=1&autoplay=1";                            // url to pass to iframe renderers
            ...
            case "vimeo";
                ...
                if (!empty($oembed)) {              // get oembed metadata
                    ...
                    $media['url']       = "https://vimeo.com/$id";                                                                           // rebuild external url
                    ...
                    $media['iframe']    = "//player.vimeo.com/video/$id?title=0&amp;byline=0&amp;portrait=0&amp;&amp;frameborder=0&amp;autoplay=1";            // url to pass to iframe renderers (like colorbox)
                    $media['api']       = $meta;                                                                                            // we include everything we got from the site API, just in case (each site formats differently)
                } else {
            ...
            case "soundcloud";
                ...
                if ($meta = json_decode(file_get_contents("https://api.soundcloud.com/resolve?client_id=".$clientid."&format=json&url=".urlencode($url)), true)) {
                //  $oembed = json_decode(file_get_contents("https://soundcloud.com/oembed?format=json&show_comments=false&auto_play=true&url=".urlencode($url)), true);
                    $media['id']        = $meta['id'];
                    ...
                    $media['iframe']    = "//w.soundcloud.com/player/?url=".$meta['uri']."?auto_play=true&show_artwork=false&show_comments=false&color=000000&show_playcount=false&sharing=false&show_user=false&liking=false";                // http://developers.soundcloud.com/docs/oembed   --  https://github.com/soundcloud/Widget-JS-API/wiki/widget-options
